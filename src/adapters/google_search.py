...
class GoogleQueryService:
    ...
    def search(self, query, query_config: Dict) -> pd.DataFrame:
        ...
        if results.empty:
            ...
            results = self._fetch_and_cache(query, query_config)
        return results
    ...
    def _fetch_and_cache(self, query, query_config) -> pd.DataFrame:
    ...
    def _cache_results(self, results: pd.DataFrame):
        cached_results = self._read_cache()
        updated_results = pd.concat((cached_results, results))
        with self._store.write("cached_search_results.csv") as h:
